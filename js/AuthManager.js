    ...
    class AuthManager {
        ...
        async syncUserDocument(user) {
            ...
            try {
                ...
                let redtootPin = snap.exists ? snap.data()?.redtootPin : "";
                if (snap.exists && !/^[0-9A-F]{8}$/.test(redtootPin || "") && this.helpers?.assignUserHexId) {
                    redtootPin = await this.helpers.assignUserHexId(user.uid, "redtootPin");
                }
                if (snap.exists) {
                    await ref.set(baseData, { merge: true });
                    this.userData = { id: snap.id, ...snap.data(), ...baseData, redtootPin };
                } else {
                    const created = {
                        ...baseData,
                        isAdmin: false,
                    ...
                    if (this.helpers?.assignUserHexId) {
                        redtootPin = await this.helpers.assignUserHexId(user.uid, "redtootPin");
                    }
                    this.userData = { id: user.uid, ...created, redtootPin };
                }
            } catch (error) {
        ...
        updateAuthUI() {
            ...
            if (this.currentUser) {
                ...
                if (userPin) userPin.textContent = this.userData?.redtootPin ? `PIN ${this.userData.redtootPin}` : "";
                ...
                if (userPin) userPin.textContent = "";
